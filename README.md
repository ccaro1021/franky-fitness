...
- **Deterministic grocery lists.** Grocery generation is plain code, not an LLM — it sums and groups ingredients across a plan by canonical name and store section. An LLM is used only for the fuzzy parts (normalizing ingredient names, reconciling units, assigning categories), and those results are cached so repeat requests are pure code.
