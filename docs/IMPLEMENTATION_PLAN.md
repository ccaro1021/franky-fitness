...
- **2026-06-15** — **Built Slice 9 (grocery normalization agent).** `backend/database.py` adds the `ingredient_normalizations` table, seeded at `setup_tables()` time from the retired `_INGREDIENT_ALIASES` dict plus self-maps for every base item and "keep as written" item (158 seed rows). `backend/grocery_agent.py` (`get_cached`/`_upsert_cache`/`normalize_ingredients`) checks the cache first and only calls `claude-opus-4-1-20250805` (forced tool call, one batch) on misses, returning a `Transcript` with `agent_type="grocery_normalizer"` (None if every name was a cache hit). `grocery.py` drops `_INGREDIENT_ALIASES` entirely; `generate_grocery_list` reads `ingredient["canonical_name"]` (falling back to `name` if absent). `meal_agent.finalize_meal_plan` no longer calls `get_recipe` — it just sets `ingredients_fetched: false` on each meal. New `backend/main._prepare_grocery_data(plan_id, plan)` (used by both the `/api/plans/{id}/grocery-list` route and the chat short-circuit) fetches ingredients for any meal missing them (treating pre-Slice-9 plans, which already have `ingredients` but no flag, as already-fetched — avoiding a needless re-fetch), normalizes any ingredient missing `canonical_name`, persists both onto `plans.content`, and returns `missing_meals` for any meal whose `get_recipe` failed (surfaced in `GroceryListCard` and the chat response). `tests/test_grocery.py` rewritten (17 -> 14 tests) to stub `canonical_name` directly instead of the alias dict, now covering summing/rounding/categorization only. New `evals/grocery_normalization.py` (4 tasks) clears the cache for a novel ingredient string and uses `judge()` to check the LLM-generated canonical name follows the audited patterns — all 4 passed on first run. **One refinement during implementation:** the few-shot prompt didn't originally cover milk fat-content variants ("2% reduced fat milk"), and the model normalized it to "2% milk" instead of the cache's "milk", which would have created an unresolvable two-entry split (cache lookups are one-hop by raw string, not by canonical name) — added an explicit milk-variants pattern to the prompt and cleared the bad cache entry. Verified end-to-end against a real saved plan (id 1, 28 ingredients across 4 meals): first grocery-list request made one Opus call (35 grocery items returned) and persisted `canonical_name`/`ingredients_fetched` onto `plans.content`; second request made zero LLM/Spoonacular calls and returned the same 35 items from cache.
