...
8. ☐ **Transcript observability + eval harness** *(Slice 8 — PLANNED)* — *full execution spec in [`SLICE_8_OBSERVABILITY_EVALS_PLAN.md`](SLICE_8_OBSERVABILITY_EVALS_PLAN.md). Two phases: (1) capture a canonical JSONL `Transcript` per agent call (system prompt, messages, tool calls, output, outcome, tokens/latency, `agent_invoked`) with `agent_runs` demoted to a queryable index (`+agent_invoked`, `+transcript_id`; no prompt content in the DB) and a `backend/view_transcript.py` CLI; (2) an `evals/` harness (Task/Trial/Grader, code-based + LLM-as-judge, pass@k/pass^k) that replays tasks through `run_coordinator` and consumes the same Transcript format. Mapped as closely as possible to Anthropic's "Demystifying Evals for AI Agents."*
