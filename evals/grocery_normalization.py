...
*novel* ingredient strings it hasn't seen before?

    python -m evals.grocery_normalization
    python -m evals.grocery_normalization --task novel_dried_herb

Each task clears any cached entry for its raw ingredient string (forcing a real
LLM call via grocery_agent.normalize_ingredients), then grades the resulting
canonical name with an LLM judge against the pattern it should follow. This is
LLM-judgment territory (real Anthropic calls) — keep the task list small, like
evals/harness.py.
"""
